...
from utils_ad import download_data, train_v4, acc_calc, pickle_dump
...
if __name__ == "__main__":
    ...
    for i, monte_carlo_seed in enumerate(monte_carlo_seeds_single):
        ...
        save_dir = curr_dir + f'/Model_{model_name}_EpochNumber_{epoch_num}_decreasing' + f'/MonteCarlo{monte_carlo_seed}'
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)

        # transfer your model to train mode
        train_v4(model = model, optimizer = optimizer, loss_func = loss,
                    train_loader = train_generator, val_loader = val_generator,
                    epoch_num = 0.1, model_name = model_name, check_dic = check_dic, device = device)
